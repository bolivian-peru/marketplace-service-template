...
export async function getUberEatsRestaurantDetails(restaurantId: string): Promise<Restaurant | null> {
  const slugOrId = restaurantId.trim();
  const url = `https://www.ubereats.com/store/${encodeURIComponent(slugOrId)}`;
  ...
  const name = html.match(/<h1[^>]*>([^<]+)<\/h1>/i)?.[1]?.trim()
    || html.match(/"title"\s*:\s*\{"text"\s*:\s*"([^"]+)"/)?.[1]
    || null;
  if (!name) return null;

  const rating = html.match(/"ratingValue"\s*:\s*([\d.]+)/)?.[1];
  const reviewCount = html.match(/"reviewCount"\s*:\s*(\d+)/)?.[1];
  const deliveryText = html.match(/(\d+[–-]\d+\s*min)/i)?.[1];
  const feeText = html.match(/delivery[^$]{0,40}\$([\d.]+)/i)?.[1] || null;
  const eta = parseDeliveryTime(deliveryText);

  return {
    id: restaurantId,
    name,
    rating: rating ? parseFloat(rating) : null,
    reviews_count: reviewCount ? parseInt(reviewCount, 10) : null,
    delivery_fee: feeText ? parseFloat(feeText) : null,
    delivery_time_min: eta.min,
    delivery_time_max: eta.max,
    minimum_order: null,
    promotions: [],
    image_url: html.match(/"heroImageUrl"\s*:\s*"([^"]+)"/)?.[1] || null,
    platform: 'ubereats',
  };
}
